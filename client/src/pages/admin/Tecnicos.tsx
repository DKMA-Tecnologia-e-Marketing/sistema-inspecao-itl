...
export default function TecnicosPage() {
  ...
  const { data: allTecnicos, isLoading, refetch } = trpc.tecnicos.list.useQuery();
  const { data: inspetores } = trpc.tecnicos.listByTipo.useQuery({ tipo: "inspetor" });
  const { data: responsaveis } = trpc.tecnicos.listByTipo.useQuery({ tipo: "responsavel" });
  ...
  const handleOpenDialog = (tecnico?: (typeof allTecnicos)[0], tipo?: "inspetor" | "responsavel") => {
    if (tecnico) {
      ...
      setFormData({
        ...
        cft: tecnico.cft || "",
        crea: tecnico.crea || "",
        tenantId: tecnico.tenantId || 0,
        ativo: tecnico.ativo ?? true,
      });
      setTipoSelecionado(tecnico.tipo);
    } else {
      const tipoFinal = tipo || tipoSelecionado;
      resetForm();
      setFormData((prev) => ({ ...prev, tipo: tipoFinal }));
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={() => handleOpenDialog(undefined, "inspetor")} variant="outline">
              <Plus className="h-4 w-4 mr-2" />
              Novo Inspetor
            </Button>
            <Button onClick={() => handleOpenDialog(undefined, "responsavel")}>
              <Plus className="h-4 w-4 mr-2" />
              Novo Responsável
            </Button>
          </div>
                      ...
                      <TableBody>
                        {filteredInspetores.map((tecnico) => (
                          <TableRow key={tecnico.id}>
                            ...
                            <TableCell>{getTenantName(tecnico.tenantId)}</TableCell>
                            <TableCell>
                              <span
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  <DialogTitle>
                    {editingTecnico
                      ...
                      : `Criar ${formData.tipo === "inspetor" ? "Inspetor" : "Responsável"} Técnico`}
                  </DialogTitle>
                  <DialogDescription>
                    {editingTecnico
                      ? "Atualize as informações do técnico"
                      : formData.tipo === "inspetor"
                        ? "Preencha os dados do Inspetor Técnico. Campos obrigatórios: Nome Completo, CPF e CFT."
                        : "Preencha os dados do Responsável Técnico. Campos obrigatórios: Nome Completo, CPF e CREA."}
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit}>
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="tenantId">Estabelecimento (Opcional)</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Selecione um estabelecimento (opcional)" />
                        </SelectTrigger>
                        <SelectContent>
