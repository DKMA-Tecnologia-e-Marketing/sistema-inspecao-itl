...
async function addInspections() {
  ...
  try {
    ...
    console.log("📋 Criando 5 novas inspeções...\n");
    
    // 1. Buscar o tenant "ITL Teste" ou o primeiro tenant disponível
    ...
    // 2. Buscar ou criar clientes e veículos
    ...
    if (existingCustomers.length >= 5) {
      ...
      customers = existingCustomers.slice(0, 5);
      console.log(`✅ Usando ${customers.length} clientes existentes`);
    } else {
      // Criar novos clientes se necessário
    ...
    // 3. Buscar ou criar veículos para cada cliente
    console.log("\n🚗 Preparando veículos...");
    const vehicles = [];
    ...
    // 4. Buscar tipos de inspeção vinculados ao tenant
    ...
    for (let i = 0; i < 5; i++) {
      // Distribuir clientes e veículos
      ...
      console.log(`   ✅ ${i + 1}/5 inspeções criadas...`);
    }
